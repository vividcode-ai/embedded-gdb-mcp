...
const source = join(__dirname, '..', 'target', 'release', `embedded-gdb-mcp${ext}`);
...
copyFileSync(source, join(destDir, `embedded-gdb-mcp${ext}`));

if (platform !== 'win32') {
  try {
    chmodSync(join(destDir, 'embedded-gdb-mcp'), 0o755);
  } catch {}
}
